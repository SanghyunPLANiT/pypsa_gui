...
export interface DemandForecastSettings {
  annualGrowthPct: number;       // % applied to all loads each year
  peakGrowthPct: number | null;  // peak envelope growth; null = same as annualGrowthPct
  floorGrowthPct: number | null; // floor growth; null = same as annualGrowthPct
  peakTouched: boolean;          // user has independently edited peakGrowthPct
  floorTouched: boolean;         // user has independently edited floorGrowthPct
}

export interface MacroSettings {
  discountRate: number;   // WACC/discount rate, e.g. 0.05
  carbonPrice: number;    // $/tCO₂ (replaces standalone carbonPrice state)
  baseYear: number;       // first investment period year
  periodCount: number;    // number of investment periods
  periodLength: number;   // years per period
}

export interface MultiYearPeriodResult {
  year: number;
  newCapacityMw: Record<string, number>;   // carrier → MW newly built this period
  totalCapacityMw: Record<string, number>; // carrier → cumulative active MW
  capexM: number;                          // annualised CAPEX $M
  opexM: number;                           // fuel + carbon OPEX $M
  avgSmpPerMwh: number;                    // period-average marginal price $/MWh
}

export interface MultiYearResults {
  type: 'multiyear';
  periods: MultiYearPeriodResult[];
  totalNpvM: number;
  narrative: string[];
  runMeta: {
